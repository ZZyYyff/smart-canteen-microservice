...
| 页面 | 路由 | 说明 |
|------|------|------|
| 仪表盘 | `/admin/home` | 系统统计（用户数、菜品数、窗口数）、模块说明、快速入口 |
| 用户管理 | `/admin/users` | 用户列表、搜索筛选、分页、启用/禁用、角色变更 |
| 菜单管理 | `/admin/menus` | 全局菜品管理，支持搜索和状态筛选 |
| 窗口管理 | `/admin/windows` | 取餐窗口新增、启用、停用 |
| 每日菜单 | `/admin/daily-menus` | 每日菜单创建、编辑、删除 |
| 系统配置 | `/admin/system` | 部署环境信息、微服务模块概览 |
...
数据库初始化脚本 (`deploy/sql/init.sql`) 已内置 3 个演示账号，密码均为 `123456`：
...
> 以上账号由 Docker 首次启动时自动创建，无需手动插入。

## 12. 学生端演示流程

> ⚠️ **重要前提**：学生端点餐依赖"今日菜单"。数据库初始化后没有默认的今日菜单数据，**必须先由商家登录并创建今日菜单**（见下方 §13 商家端演示流程第 2-3 步），否则学生端"今日菜单"页面为空。
...
> ⚠️ **重要**：商家在进行订单处理前，需先创建今日菜单，否则学生端无法看到菜品。

1. 使用商家账号登录，自动进入商家后台
2. 点击左侧"每日菜单"，点击"新增"，选择当天日期和餐段，勾选菜品，创建今日菜单
3. 确认菜单状态为"生效中"
4. 首页可看到今日订单统计和最近订单
5. 点击左侧"菜品管理"，可新增菜品（填写名称、价格、库存等）
6. 菜品创建后默认为上架状态，可点击"下架"暂停售卖
7. 点击左侧"订单管理"进入订单处理页
8. 选择"待接单" Tab，点击"接单"按钮
9. 切换到"已接单" Tab，点击"开始制作"
10. 切换到"制作中" Tab，点击"制作完成"（此时订单进入取餐队列）
11. 点击左侧"叫号核销"进入取餐管理页
12. 选择取餐窗口，点击"叫下一号"
13. 此时大屏接收到 WebSocket 推送，显示叫号信息
14. 学生出示取餐码，在右侧输入取餐号和取餐码，点击"确认核销"
